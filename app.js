...
function renderAdminFaculty() {
  const tbody = document.getElementById('adm-faculty-table-body');
  if (!tbody) return;

  tbody.innerHTML = state.faculty.map(f => `
    <tr class="border-b border-slate-100 hover:bg-slate-50 text-xs">
      <td class="py-3 px-4 flex items-center gap-2">
        <img src="${escapeHtml(f.photo)}" class="w-8 h-8 rounded-full object-cover border" onerror="this.src='https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=100&auto=format&fit=crop&q=80'">
        <span class="font-bold text-slate-800">${escapeHtml(f.name)}</span>
      </td>
      <td class="py-3 px-4 text-amber-600 font-semibold">${escapeHtml(f.role)}</td>
      <td class="py-3 px-4 text-slate-600">${escapeHtml(f.subject)}</td>
      <td class="py-3 px-4 text-slate-500">${escapeHtml(f.qualification)}</td>
      <td class="py-3 px-4 text-right">
        <button onclick="deleteFaculty('${f.id}')" class="text-red-600 hover:text-red-800 font-semibold px-2 py-1 rounded bg-red-50 hover:bg-red-100 transition">
          Delete
        </button>
      </td>
    </tr>
...
function handleAddFaculty(e) {
  ...
  const role = document.getElementById('fac-form-role').value.trim();
  const subject = document.getElementById('fac-form-subject').value.trim();
  const qualification = document.getElementById('fac-form-qual').value.trim();
  let photo = document.getElementById('fac-form-photo-url').value.trim();

  const fileInput = document.getElementById('fac-form-file');
  if (fileInput && fileInput.files && fileInput.files[0]) {
    const reader = new FileReader();
    reader.onload = function(evt) {
      photo = evt.target.result;
      saveNewFaculty({ name, role, subject, qualification, photo });
    };
    reader.readAsDataURL(fileInput.files[0]);
  } else {
    if (!photo) {
...
function saveNewFaculty(data) {
  ...
  showToast(`Faculty member "${data.name}" added!`, "success");
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const facForm = document.getElementById('add-faculty-form');
  if (facForm) facForm.addEventListener('submit', handleAddFaculty);
